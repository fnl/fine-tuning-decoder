...
- **Facts worth re-checking in the notebook before trusting the T4 plan**: Colab's live torch version and driver (ticket 05's snapshot was 2026.07); no first-hand report of vLLM 0.29.0 on a Colab T4 exists — hence the fallback order. *Update 2026-09-21*: the vLLM 0.29.0 install and the restart flow work first-hand on a free T4 (ticket 05 comments); generation is still unverified.
