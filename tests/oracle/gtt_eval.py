# Vendored verbatim from https://github.com/xinyadu/gtt/blob/master/eval.py
# (MIT License, Copyright (c) 2021 xinyadu) as the parity oracle for src/eval.py.
# Only change: the unused `import numpy as np` line was removed so numpy is not a
# test dependency. Beware: `eval_tf` has a mutable default `docids=[]`; always pass
# `docids` explicitly when calling it more than once per process.
import re
import string
import json
import argparse
